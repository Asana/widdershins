...
function schemaToArray(schema,offset,options,data) {
    ...
    walkSchema(schema,{},wsState,function(schema,parent,state){
        ...
        let restrictionKeysToRemove = [];
        for (let key in depthRestrictionMap) {
            if (!depthRestrictionMap.hasOwnProperty(key)) {
                continue;
            }

            if (entry.depth > key) {
                entry.restrictions = depthRestrictionMap[key];
            } else {
                restrictionKeysToRemove.push(key);
            }
        }

        for (let i = 0; i < restrictionKeysToRemove; i++) {
            delete depthRestrictionMap[restrictionKeysToRemove[i]];
        }
        ...
        if (entry.restrictions) {
            depthRestrictionMap[entry.depth] = entry.restrictions;
        }
