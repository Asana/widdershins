        ...
        if ((param.schema.type === 'object') && (data.options.expandBody || (!param.schema["x-widdershins-oldRef"]))) {
            ...
            for (let block of props) {
                for (let prop of block.rows) {
                    if (prop.restrictions && prop.restrictions.includes("read-only")) {
                        continue;
                    }
                    let param = {};
                    param.in = 'body';
...
function getResponses(data) {
    ...
    for (let r in data.operation.responses) {
        ...
        for (let ct in response.content) {
            ...
            if (
                (
                    (contentType.schema && contentType.schema["x-widdershins-oldRef"] && contentType.schema["x-widdershins-oldRef"].startsWith('#/components/')) ||
                    (contentType.schema && contentType.schema.properties && contentType.schema.properties.data && contentType.schema.properties.data["x-widdershins-oldRef"] && contentType.schema.properties.data["x-widdershins-oldRef"].startsWith('#/components/')) ||
                    (contentType.schema && contentType.schema.properties && contentType.schema.properties.data && contentType.schema.properties.data.items["x-widdershins-oldRef"] && contentType.schema.properties.data.items["x-widdershins-oldRef"].startsWith('#/components/'))
                )
            ) {
                let schemaName;
                if (contentType.schema && contentType.schema.properties && contentType.schema.properties.data) {
...
function convert(api, options, callback) {
    api.components.schemas = resolveSchema(api, api.components.schemas, '#/components/schemas', false);
    for (let path in api.paths) {
        if (!api.paths.hasOwnProperty(path)) {
            continue;
        }
        for (let method in api.paths[path]) {
            if (!api.paths[path].hasOwnProperty(method)) {
                continue;
            }

            if (api.paths[path][method]["requestBody"]) {
                api.paths[path][method]["requestBody"] = resolveSchema(api, api.paths[path][method]["requestBody"], '#/paths/' + path + '/' + method + '/requestBody', false);
            }
        }
    }

    if (options.resolve) {
        swagger2openapi.convertObj(api, {
            resolve: true,
            source: options.source,
            verbose: options.verbose
        }, function (err, sOptions) {
            if (err) {
...
function resolveSchema(api, schema, path, ignoreRefs) {
    if (!isObject(schema)) {
        return schema;
    }
    ...
    for (let key in schema) {
        ...
        if (key === 'x-docs-overrides') {
            for (let overrideKey in schema[key]) {
                if (!schema[key].hasOwnProperty(overrideKey)) {
                    continue;
                }
                let overridePath = overrideKey.split('.');
                let overrideCurrent = schema;
                for (let i = 0; i < overridePath.length; i++) {
                    if (!overrideCurrent.hasOwnProperty(overridePath[i])) {
                        overrideCurrent[overridePath[i]] = {}
                    }

                    if (i === overridePath.length - 1) {
                        overrideCurrent[overridePath[i]] = schema[key][overrideKey];
                    } else {
                        overrideCurrent = overrideCurrent[overridePath[i]];
                    }
                }
            }
        }

        if (!ignoreRefs && key === '$ref') {
            return resolveSchema(api, jptr(api, schema[key]), schema[key], false);
        }

        if (key === 'allOf') {
            let result = {};
            for (let i = 0; i < schema[key].length; i++) {
                mergeDeep(result, resolveSchema(api, jptr(api, path + '/' + key + '/' + i), path + '/' + key + '/' + i, false));
            }
            return result;
        }

        schema[key] = resolveSchema(api, schema[key], path + '/' + key.replaceAll("/", "~1"), ignoreRefs);
    }

    return schema;// sort(schema);
}
...
function sort(schema) {
    ...
    for (let i = 0; i < result.length; i++) {
        let insertAfter = result[i].value["x-insert-after"];
    ...
    for (let i = 0; i < result.length; i++) {
        schema[result[i].key] = result[i].value;
    }
