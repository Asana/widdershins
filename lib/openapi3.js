...
function convert(api, options, callback) {
    api.components.schemas = resolveSchema(api, api.components.schemas, '#/components/schemas');
...
function resolveSchema(api, schema, path) {
    if (!isObject(schema)) {
        return schema;
    }
    ...
    for (let key in schema) {
        ...
        if (key === '$ref') {
            return resolveSchema(api, jptr(api, schema[key]), schema[key]);
        }

        if (key === 'allOf') {
            let result = {};
            for (let i = 0; i < schema[key].length; i++) {
                mergeDeep(result, resolveSchema(api, jptr(api, path + '/allOf/' + i), path));
            }
            return result;
        }

        schema[key] = resolveSchema(api, schema[key], path + '/' + key);
    }
