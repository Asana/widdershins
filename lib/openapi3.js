        ...
        for (var rb in op.requestBody.content) {
            ...
            if (!data.bodyParameter.exampleValues.object) {
                ...
                if (op.requestBody.content[rb].examples) {
                    ...
                    if (data.operation.operationId === "batchRequest") {
                        data.options.maxDepth = data.options.maxDepth + 1;
                    }
...
function convert(api, options, callback) {
    ...
    api.paths = resolveSchema(api, api.paths, '#/paths', true);
    ...
    for (let key in schema) {
        ...
        if (key === 'allOf') {
            ...
            for (let i = 0; i < schema[key].length; i++) {
                mergeDeep(result, resolveSchema(api, jptr(api, path + '/' + key + '/' + i), path, false));
            }
            return result;
        }
